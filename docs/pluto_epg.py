...
def _fetch_timelines(pluto_id: str, window_start: float, window_end: float) -> list[dict[str, Any]]:
    ...
    for t in timelines:
        ...
        title = str((ep or {}).get("name") or t.get("title") or "").strip()
        if not title:
            continue
        ...
        for src in (
            (ep or {}).get("poster"),
            (ep or {}).get("series", {}).get("tile") if isinstance((ep or {}).get("series"), dict) else None,
            (ep or {}).get("thumbnail"),
        ):
        ...
        for src in (
            ...
            (ep or {}).get("series", {}).get("featuredImage")
            if isinstance((ep or {}).get("series"), dict)
            else None,
        ):
            if isinstance(src, dict):
                path = str(src.get("path") or "").strip()
        ...
        for raw in (
            ...
            if m:
                year = int(m.group(0))
                break
        # Film channels: treat as movie when series.type == film
